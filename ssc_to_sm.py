...
def _prompt_for_directory() -> Optional[Path]:
    print(
        "Enter the folder that contains your .ssc files (subfolders are included).\n"
        "You can paste a path like C\\Users\\Malloy\\Downloads\\Tech-Bit Adventures",
        file=sys.stdout,
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Path to a folder containing SSC files (conversion includes subfolders)."
    ...
    path = args.path or _prompt_for_directory()
    if path is None:
        LOGGER.info("No path provided. Exiting without conversion.")
        return 0

    if not path.exists():
        LOGGER.error("Provided path does not exist: %s", path)
        return 1
    if not path.is_dir():
        LOGGER.error("Provided path is not a directory: %s", path)
        return 1

    convert_directory(path, log_file)
    return 0
